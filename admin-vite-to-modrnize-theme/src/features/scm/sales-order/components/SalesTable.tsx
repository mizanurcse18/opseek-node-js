...
export default function SalesTable({ onAdd, onEdit, isSuperUser = false, onRefresh }: SalesTableProps) {
  ...
  const { buttons } = useMenuButtons(React.useMemo(() => [
    { button_id: 'btnAdd',    button_title: 'Add Sales Order' },
    { button_id: 'btnEdit',   button_title: 'Edit Sales Order' },
    { button_id: 'btnDelete', button_title: 'Delete Sales Order' },
  ], []));
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await salesService.delete(itemToDelete.sales_id);
      if (res && (res.status_code === 200 || res.response_code === 'SUCCESS' || res.response_code === 'Success')) {
        toast({ title: 'Success', description: 'Sales order deleted successfully.', status: 'success' });
  ...
  const fetchDataFn = React.useMemo(() =>
    isSuperUser ? salesService.getGridDataSuper : salesService.getGridData,
  [isSuperUser]);
    ...
    },
    {
      ...
      render: (_: any, row: any) => (
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-text-main text-[11px]">{row.sales_no}</span>
        </div>
      ),
    },
    {
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        description={`Are you sure you want to delete sales order "${itemToDelete?.sales_no}"? This action cannot be undone.`}
        confirmLabel="Delete"
