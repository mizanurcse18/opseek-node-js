...
export default function PurchaseRequisitionPage({ isSuperUser = false }: PurchaseRequisitionPageProps) {
  ...
  const handleAdd = useCallback(() => { setSelectedPR(null); setIsModalOpen(true); }, []);
  const handleEdit = useCallback((pr: any) => { setSelectedPR(pr); setIsModalOpen(true); }, []);
  const handleClose = useCallback(() => { setIsModalOpen(false); setSelectedPR(null); }, []);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 sm:space-y-0">
        <div>
          ...
          <p className="text-xs font-medium text-text-muted mt-1 uppercase tracking-wider">
            Create and manage purchase requisitions with line items.
          </p>
        </div>
