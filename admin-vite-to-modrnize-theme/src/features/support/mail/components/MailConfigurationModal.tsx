...
interface MailConfigurationModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave?: () => void;
  initialData?: any;
...
export function MailConfigurationModal({ isOpen, onClose, onSave, initialData, isSuperUser }: MailConfigurationModalProps) {
  const [loading, setLoading] = React.useState(false);
  const isEditing = !!(initialData?.configId || initialData?.config_id);
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      maxWidth="lg"
      title={isEditing ? 'Edit Mail Configuration' : 'Create Mail Configuration'}
      headerAction={
