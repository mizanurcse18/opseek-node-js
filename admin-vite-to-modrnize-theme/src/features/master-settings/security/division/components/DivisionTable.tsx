...
import { Edit2, Trash2, Plus, Map, History, Layout } from 'lucide-react';
...
export default function DivisionTable({ onAdd, onEdit, isSuperUser = false }: DivisionTableProps) {
    ...
    if (isSuperUser) {
      const loadCompanies = async () => {
        try {
          ...
          console.error('Failed to load companies for DivisionTable:', error);
        }
      };
      loadCompanies();
    }
  ...
  const { buttons } = useMenuButtons(React.useMemo(() => [
    { button_id: 'btnAdd', button_title: 'Add Division' },
    { button_id: 'btnEdit', button_title: 'Edit Division' },
    { button_id: 'btnDelete', button_title: 'Delete Division' },
  ...
  const handleConfirmDelete = async () => {
    ...
    const id = itemToDelete.DivisionID || itemToDelete.division_id;
    setIsDeleting(true);
    try {
      const response = await geoService.deleteDivision(id);
      if (response && (response.status_code === 200 || response.response_code === 'Success')) {
        toast({ title: 'Success', description: 'Division deleted successfully.', status: 'success' });
  ...
  const fetchDataFn = React.useMemo(() => 
    isSuperUser ? geoService.getDivisionGridDataSuper : geoService.getDivisionGridData,
  [isSuperUser]);
    ...
    },
    {
      ...
      render: (_: any, row: any) => (
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-text-main">{row.division_name || row.DivisionName}</span>
        </div>
      )
    },
    {
      header: 'Bangla Name',
      accessor: 'bangla_division_name',
      searchFieldName: 'bangla_division_name',
      sortable: true,
      searchable: true,
      visible: false,
      className: 'text-text-muted',
      render: (_: any, row: any) => (
        <span className="font-medium text-text-muted">{row.bangla_division_name || row.BanglaDivisionName || '—'}</span>
      )
    },
    {
      header: 'Company',
          ...
          {btnAuditGrid?.visible && (
            <Button
              ...
              onClick={() => setAuditLogState({
                isOpen: true,
                id: row.DivisionID || row.division_id,
                name: row.division_name || row.DivisionName
              })}
            >
