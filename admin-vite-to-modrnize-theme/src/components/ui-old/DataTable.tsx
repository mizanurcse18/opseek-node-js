    ...
    <div className="flex flex-col gap-3">
        ...
        <div className="flex items-center gap-2 bg-content-bg rounded-lg px-3 py-1.5 border border-border-theme">
          ...
          >
            ...
            {columns.filter(c => c.searchable).map(col => (
              <option key={col.accessor} value={col.accessor} className="bg-card-bg text-text-main">{col.header}</option>
            ))}
          </select>
          <input 
            type="text" 
      ...
      <div className="flex items-center justify-between bg-card-bg px-3 py-1.5 rounded-xl border border-border-theme shadow-sm">
        <div className="flex items-center gap-2">
          <Button 
            ...
            disabled={loading}
            title="Reload Grid"
            className="h-7 w-7 p-0 rounded-lg text-text-muted hover:text-primary-600 hover:bg-primary-500/10 transition-all border border-border-theme"
          >
            <RotateCw className={cn("h-3 w-3", loading && "animate-spin")} />
          </Button>
          <span className="text-[9px] font-black uppercase tracking-widest text-text-muted/50 ml-1">Grid toolbar</span>
        </div>
        ...
        <div className="flex items-center gap-3">
          {showExport && (
            <Button 
              variant="outline" 
              size="sm" 
              className="h-7 text-[9px] font-black uppercase tracking-widest gap-2 bg-card-bg border-border-theme hover:bg-content-bg hidden sm:flex"
            >
        ...
        <div className="overflow-x-auto no-scrollbar">
          <Table className="min-w-[800px] relative border-collapse">
            <TableHeader className="sticky top-0 bg-card-bg z-10 shadow-sm">
              <TableRow className="bg-content-bg/50 border-b-2 border-border-theme h-10">
                {columns.filter(c => c.visible !== false).map((col) => (
                  <TableHead 
                    key={col.accessor} 
                    className={cn(
                      "text-[10px] font-black uppercase tracking-widest text-text-main py-2 px-3",
                      col.sortable && "cursor-pointer select-none hover:bg-content-bg/80 transition-colors"
                    )}
                    ...
                    {col.searchable ? (
                      <div className="relative">
                        {col.searchType === 'select' ? (
                          ...
                          >
                            <option value="" className="bg-card-bg">All</option>
                            {col.searchOptions?.map(opt => (
                              <option key={opt.value} value={opt.value} className="bg-card-bg">{opt.label}</option>
                            ))}
                          </select>
                        ) : (
                          <>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={columns.filter(c => c.visible !== false).length} className="h-32 text-center">
                    <div className="flex flex-col items-center justify-center gap-2 text-red-500">
                      <span className="text-[9px] font-bold uppercase tracking-widest border border-red-500/20 bg-red-500/10 px-3 py-1.5 rounded-lg">
                        Error: {error}
                      </span>
                    </div>
                  </TableCell>
                ...
                <TableRow>
                  <TableCell colSpan={columns.filter(c => c.visible !== false).length} className="h-32 text-center text-text-muted font-medium italic text-[11px]">
                    {emptyMessage}
                  </TableCell>
                </TableRow>
              ) : (
                data.map((row, index) => (
                  <TableRow 
                    ...
                    className={cn(
                      "hover:bg-content-bg/30 transition-colors border-b border-border-theme last:border-b-0 text-text-main",
                      striped && index % 2 === 1 && "bg-content-bg/10"
                    )}
                  >
                    {columns.filter(c => c.visible !== false).map((col) => (
      ...
      <div className="md:hidden flex flex-col gap-4">
        {loading ? (
          ...
          <div className="h-32 flex items-center justify-center bg-red-500/5 rounded-xl border border-red-500/20 p-4">
          ...
          data.map((row, index) => (
            <div key={row.id || row.security_rule_id || index} className="bg-card-bg rounded-2xl border border-border-theme shadow-lg shadow-black/5 overflow-hidden flex flex-col transition-all active:scale-[0.98]">
              ...
              {/* Card Body Grid */}
              <div className="px-4 py-3 bg-content-bg/20 grid grid-cols-2 gap-y-3 gap-x-4 border-y border-border-theme/50 text-text-main">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-1 py-1 bg-card-bg md:bg-transparent rounded-xl border md:border-0 border-border-theme shadow-sm md:shadow-none">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 border-l border-border-theme pl-3 hidden sm:flex">
              ...
              <select 
                ...
                className="bg-content-bg border border-border-theme text-text-main text-[9px] font-black rounded-md focus:ring-primary-600/50 focus:border-primary-600 block p-0.5 px-1 outline-none cursor-pointer transition-all hover:bg-card-bg"
              >
                {[10, 20, 50, 100].map(size => (
                  <option key={size} value={size} className="bg-card-bg">{size}</option>
                ))}
              </select>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 w-full sm:w-auto justify-between sm:justify-end">
            <Button
              ...
              disabled={page === 1}
              className="h-8 sm:h-7 px-4 sm:px-3 text-[9px] font-black uppercase tracking-tighter shadow-sm border-border-theme hover:bg-content-bg"
            >
              Prev
            </Button>
            ...
            <div className="flex items-center gap-1 px-3 min-w-[70px] justify-center">
              ...
              <span className="text-[8px] font-black text-text-muted/30 uppercase tracking-widest mx-0.5">/</span>
