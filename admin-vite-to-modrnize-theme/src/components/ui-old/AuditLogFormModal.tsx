      ...
      <div className="flex flex-col min-h-[500px]">
        {loading ? (
          <div className="flex-1 flex flex-col items-center justify-center py-20 gap-4">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest text-text-muted/50">Restoring version data...</p>
          </div>
        ) : !currentLog ? (
           <div className="flex-1 flex items-center justify-center text-text-muted">No logs found.</div>
        ) : (
          <div className="flex-1 space-y-6 py-2 overflow-y-auto no-scrollbar">
            {/* Version Header */}
            <div className="flex flex-wrap items-center justify-between gap-4 bg-content-bg/30 p-4 rounded-xl border border-border-theme">
               <div className="flex items-center gap-3">
                  <div className={cn(
                    "flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-[10px] font-black uppercase tracking-widest shadow-sm",
                    currentLog.row_state === 'Added' ? 'text-emerald-600 bg-emerald-500/10 border-emerald-500/20' :
                    currentLog.row_state === 'Modified' ? 'text-amber-600 bg-amber-500/10 border-amber-500/20' :
               ...
               {Object.entries(parsedValues).map(([key, value]) => (
                  <div key={key} className="space-y-2">
                    <div className="flex items-center gap-1.5">
                       ...
                       <Label className="text-[10px] font-black uppercase tracking-widest text-text-muted/50">
                         {key.replace(/_/g, ' ')}
                       </Label>
                    </div>
                    {typeof value === 'string' && value.length > 50 ? (
                      <Textarea 
                        readOnly 
                        value={value} 
                        className="bg-content-bg/20 text-[11px] font-medium text-text-main h-24 border-border-theme resize-none cursor-default" 
                      />
                    ) : (
                      <Input 
                        readOnly 
                        value={typeof value === 'boolean' ? (value ? 'Enabled' : 'Disabled') : String(value)} 
                        className="bg-content-bg/20 text-[11px] font-medium text-text-main h-10 border-border-theme cursor-default" 
                      />
                    )}
                  </div>
               ))}
        ...
        {!loading && logs.length > 0 && (
          <div className="mt-8 pt-4 border-t border-border-theme flex items-center justify-between">
            <Button
              variant="outline"
              size="sm"
              onClick={handleNext}
              disabled={currentIndex === logs.length - 1}
              className="h-9 px-4 text-[10px] font-black uppercase tracking-widest gap-2 bg-card-bg border-border-theme hover:bg-content-bg"
            >
