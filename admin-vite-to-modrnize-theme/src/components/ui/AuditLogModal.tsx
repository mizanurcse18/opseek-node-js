  ...
  const columns: Column[] = useMemo(() => [
    {
       ...
       render: (val: string) => {
         ...
         }[val] || 'bg-gray-50 text-gray-700 ring-gray-600/20';
    ...
    },
    {
      ...
      render: (val: string) => {
        try {
          ...
          return (
            <div className="flex flex-wrap gap-2 py-1">
              {Object.entries(parsed).map(([key, value]) => (
                <div key={key} className="flex flex-col bg-gray-50/50 border border-gray-100 rounded p-1.5 min-w-[120px]">
                  <span className="text-[8px] font-black uppercase tracking-tighter text-gray-400 leading-none mb-1">{key.replace(/_/g, ' ')}</span>
          ...
          return <span className="text-[10px] text-gray-500 font-mono italic">{val}</span>;
        }
      }
    },
    {
      ...
      render: (val) => (
        <span className="text-[10px] font-black uppercase text-gray-400 tracking-widest">{val}</span>
      )
    },
    {
      header: 'Created By',
      ...
      render: (val) => (
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] font-bold text-gray-700">User #{val}</span>
        </div>
      )
    },
    {
